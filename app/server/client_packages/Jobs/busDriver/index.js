...
function setAvailableLeaveBus(state) {
    if (state === void 0) { state = false; }
    job.playerCanLeaveBus = state;
}
mp.events.add('playerEnterColshape', function (colshape) {
    if (colshape != job.checkpoint.colshape)
        return;
    if (player.vehicle) {
...
mp.events.add('C_Job-busDriver__playerRentBus', function (route) {
    loadRoute(route);
});
mp.events.add('C_Job-busDriver__playerSitBus', function () {
    if (wasStartedCounter())
        resetCounter();
});
mp.events.add('C_Job-busDriver__playerLeaveBus', function () {
    startCounter();
});
...
function resetCounter(fix) {
    if (fix === void 0) { fix = false; }
    mp.events.callRemote('S_Job-busDriver__setCounter', false);
    clearInterval(counter);
    currentTime = time;
    if (fix)
        currentTime = time + 1; // Фикс бага, когда таймер уже должен был завершиться, но он выполняется ещё раз, когда арендованный автобус уже зареспавнился
}
